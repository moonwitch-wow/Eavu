...
if not db.fps_ms == nil or db.fps_ms > 0 then
	...
	Text:SetFont(EavuDB["datatext"].font, EavuDB["datatext"].fontsize)
	...
	local function Update(self, t)
		int = int - t
		if int < 0 then
			Text:SetText(floor(GetFramerate()).." fps & "..select(3, GetNetStats()).." ms")
			int = 1
		end	
	end
	...
	Stat:SetScript("OnEnter", function(self)
		if not InCombatLockdown() then
			...
			if online > 0 then
				...
				GameTooltip:SetPoint("BOTTOM", self, "TOP", 0, Stats.TTSpacing)
				GameTooltip:ClearLines()
				GameTooltip:AddDoubleLine("Friends list:", format("Online: " .. "%s/%s",online,total),tthead.r,tthead.g,tthead.b,tthead.r,tthead.g,tthead.b)
				GameTooltip:AddLine' '
...
if not db.dur == nil or db.dur > 0 then
	...
	local function OnEvent(self)
		for i = 1, 11 do
			if GetInventoryItemLink("player", EavuDB.Slots[i][1]) ~= nil then
				current, max = GetInventoryItemDurability(EavuDB.Slots[i][1])
				if current then 
					EavuDB.Slots[i][3] = current/max
					Total = Total + 1
				end
			end
		end
		table.sort(EavuDB.Slots, function(a, b) return a[3] < b[3] end)
		
		if Total > 0 then
			Text:SetText(floor(EavuDB.Slots[1][3]*100).."% |cffffffff".."Armor".."|r")
		else
			Text:SetText("100% |cffffffff".."Armor".."|r")
		end
		...
		self:SetScript("OnEnter", function()
			if not InCombatLockdown() then
				...
				GameTooltip:ClearLines()
				for i = 1, 11 do
					if EavuDB.Slots[i][3] ~= 1000 then
						green = EavuDB.Slots[i][3]*2
						red = 1 - green
						GameTooltip:AddDoubleLine(EavuDB.Slots[i][2], floor(EavuDB.Slots[i][3]*100).."%",1 ,1 , 1, red + 1, green, 0)
					end
				end
				GameTooltip:Show()
			end
...
if not db.gold == nil or db.gold > 0 then
	...
	local function OnEvent(self, event)
		...
		if (EavuData == nil) then EavuData = {}; end
		if (EavuData.gold == nil) then EavuData.gold = {}; end
		if (EavuData.gold[myPlayerRealm]==nil) then EavuData.gold[myPlayerRealm]={}; end
		EavuData.gold[myPlayerRealm][myPlayerName] = GetMoney();
		...
		self:SetScript("OnEnter", function()
			if not InCombatLockdown() then
				...
				local thisRealmList = EavuData.gold[myPlayerRealm];
...
if not db.wowtime == nil or db.wowtime > 0 then
	...
	local function Update(self, t)
		...
		int = int - t
		if int < 0 then
			if EavuDB["datatext"].localtime == true then
				Hr24 = tonumber(date("%H"))
				Hr = tonumber(date("%I"))
				Min = date("%M")
				if EavuDB["datatext"].time24 == true then
...
if not db.avd == nil or db.avd > 0 then
	...
	if int > 0 then
		...
		if leveldifference >= 0 then
			...
			Text:SetText("avd: ".."|r"..format("%.2f", avoidance))
		else
...
if not db.armor == nil or db.armor > 0 then
	...
	if int > 0 then
		...
		Text:SetText((effectiveArmor).." ".."Armor")
	end
	--Setup Armor Tooltip
	self:SetAllPoints(Text)
	self:SetScript("OnEnter", function()
...
if EavuDB["datatext"].battleground == true then
	local Stat = CreateFrame("Frame")
	Stat:EnableMouse(true)
	
	local Text1  = InfoLeftBattleGround:CreateFontString(nil, "OVERLAY")
	Text1:SetFont(EavuDB["datatext"].font, EavuDB["datatext"].fontsize)
	...
	Text2:SetFont(EavuDB["datatext"].font, EavuDB["datatext"].fontsize)
	...
	Text3:SetFont(EavuDB["datatext"].font, EavuDB["datatext"].fontsize)
		...
		if int < 0 then
			...
			for i=1, numScores do
				...
				if healingDone > damageDone then
					dmgtxt = ("Healing : "..healingDone)
				else
					dmgtxt = ("Damage : "..damageDone)
				end
				if ( name ) then
					if ( name == UnitName("player") ) then
						Text2:SetText("Honor : "..honorGained)
						Text1:SetText(dmgtxt)
						Text3:SetText("Killing Blows : "..killingBlows)
					end   
				end
			end 
			int  = 1
